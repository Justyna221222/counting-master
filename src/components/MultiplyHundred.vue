...
<script>
import { generate_multiply100 } from '../tasks.js';
export default {
    data() {
        return {
            game: 'Multiply up to 100',
            task: null,
            state: null,
            correct_answers: 0,
            current_answer: "",
    ...
    computed: {
       current_exp() {
        ...
        correctExp() {
            let d1 = this.task[1].toString();
            let d2 = this.task[2].toString();
            return d1 + " x " + d2 + " = " + d1*d2;
        },
        mistake() {
           return this.state == 3;
         },
    ...
    methods: {
        load_task() {
            this.task = generate_multiply100();
            this.state = 0;
        },
        keyPressed(keyEvent) {
